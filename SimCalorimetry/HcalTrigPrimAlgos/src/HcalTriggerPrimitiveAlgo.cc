...
HcalTriggerPrimitiveAlgo::HcalTriggerPrimitiveAlgo( bool pf, const std::vector<double>& w, int latency,
                                                    ...
                                                    uint32_t minSignalThreshold, uint32_t PMT_NoiseThreshold, bool printCSVFile
                                                    )
...
void HcalTriggerPrimitiveAlgo::addSignal(const HBHEDataFrame & frame) {
   ...
   if (frame.id().ieta() == MYIETA && frame.id().iphi() == MYIPHI)
   {
     cout << "in addSignal(const HBHEDataFrame & frame) for " << MYIETA << " " << MYIPHI << endl;

     cout << "here is the frame in ADC:" << endl;
     for (int i=0; i < frame.size(); i++) {
       cout << frame[i].adc() << " ";
     }
     cout << endl;

     cout << "and here is the sample after linearization:" << endl;
     for(int i = 0; i < samples1.size(); ++i) {
         cout << samples1[i] << " ";
     }
     cout << endl;
   ...
   if(cPrintCSVFile == true)
   {
     ...
     cout << "| " << frame.id().ieta() << " " << frame.id().iphi() << " " << frame.id().depth() << " " << frame.id().subdet();
     if (ids.size() == 1) {
...
void HcalTriggerPrimitiveAlgo::addSignal(const HFDataFrame & frame) {
   ...
   if(frame.id().depth() == 1 || frame.id().depth() == 2) {
      ...
      for (it = ids.begin(); it != ids.end(); ++it) {
         ...
         if (frame.id().ieta() == MYIETA && frame.id().iphi() == MYIPHI)
         {
           cout << "in addSignal(const HFDataFrame & frame) for " << MYIETA << " " << MYIPHI << endl;
           ...
           cout << "here is the zerosamples that is addSignal'ed:" << endl;
           for(int i = 0; i < zero_samples.size(); ++i) {
             cout << zero_samples[i] << " ";
           }
           cout << endl;

           cout << "and here is the sample that got linearized:" << endl;
...
HcalTriggerPrimitiveAlgo::addSignal(const QIE10DataFrame& frame)
{
   ...
   for (const auto& id: ids) {
      ...
      // my
      if (detId.ieta() == MYIETA && detId.iphi() == MYIPHI)
      {
        cout << "in addSignal(const QIE10DataFrame& frame) for " << MYIETA << " " << MYIPHI << endl;
      ...
      if(cPrintCSVFile == true)
      {
        ...
        cout << "| " << detId.ieta() << " " << detId.iphi() << " " << detId.depth() << " " << detId.subdet();
        cout << " | " << id.ieta() << " " << id.iphi();
        cout << endl;
      }
      // end my
...
{  
   ...
   if (detId.ieta() == MYIETA && detId.iphi() == MYIPHI)
   {
     cout << "in addSignal(const QIE11DataFrame& frame) for " << MYIETA << " " << MYIPHI << endl;
     cout << "depth " << detId.depth() << endl;
   ...
   if(ids.size() == 2) {
      ...
      // my print
      if (detId.ieta() == MYIETA && detId.iphi() == MYIPHI) {
        cout << "ids.size() == 2, so here is the second sample:" << endl;
...
void HcalTriggerPrimitiveAlgo::addSignal(const IntegerCaloSamples & samples) {
   ...
   if (id.ieta() == MYIETA && id.iphi() == MYIPHI)
   {
     cout << "+in addSignal(const IntegerCaloSamples & samples) for " << MYIETA << " " << MYIPHI << endl;
     cout << "depth " << id.depth() << endl;
     cout << "here is the sample being added to theSumMap:" << endl;
     for(int i = 0; i < samples.size(); ++i) {
         cout << samples[i] << " ";
     }
     cout << endl;
     if (mybool) cout << "This is adding to an *EXISTING* entry" << endl;
     else cout << "This is a *NEW* entry" << endl;
     cout << "done method\n" << endl;
   } 

   if(cPrintCSVFile == true and false)
   {
     cout << RUN << " " << LUMI << " " << EVENT << " | ";
     cout << "IntegerCaloSamples | ";
     for (int i=0; i < samples.size(); i++) {
         cout << samples[i] << " ";
     }
     cout << "| " << id.ieta() << " " << id.iphi();
...
void HcalTriggerPrimitiveAlgo::analyze(IntegerCaloSamples & samples, HcalTriggerPrimitiveDigi & result) {
   ...
   if (id.ieta() == MYIETA && id.iphi() == MYIPHI)
   {
     cout << "in analyze(IntegerCaloSamples & samples, HcalTriggerPrimitiveDigi & result) for " << MYIETA << " " << MYIPHI << endl;

     cout << "here is the sample after processing (peak + slide) but before handing off to the outcoder:" << endl;
   ...
   if (id.ieta() == MYIETA && id.iphi() == MYIPHI)
   {
     cout << "and here is the now compressed HcalTriggerPrimitiveDigi destined to wind up in the output of the algorithm:" << endl;
     for(int i = 0; i < result.size(); i++) {
       cout << result.sample(i).compressedEt() << " ";
     }
     cout << endl;
   }
}
...
{
   HcalTrigTowerDetId myid(samples.id());
   if (myid.ieta() == MYIETA && myid.iphi() == MYIPHI)
   {
     cout << "in analyzeQIE11(IntegerCaloSamples& samples, HcalTriggerPrimitiveDigi& result, const HcalFinegrainBit& fg_algo) for " << MYIETA << " " << MYIPHI << endl;

     cout << "here is the IntegerCaloSample originally handed to analyze():" << endl;
     for(int i = 0; i < samples.size(); i++) {
         cout << samples[i] << " ";
     }
     cout << endl;
   ...
   if (myid.ieta() == MYIETA && myid.iphi() == MYIPHI)
   {
      ...
      cout << "digi soi " << dgPresamples << endl;
      cout << "tp soi " << tpPresamples << endl;
      cout << "digi num " << dgSamples << endl;
      cout << "tp num " << tpSamples << endl;
      cout << endl;
   }
