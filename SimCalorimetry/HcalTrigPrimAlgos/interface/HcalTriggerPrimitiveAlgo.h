...
public:
  HcalTriggerPrimitiveAlgo(bool pf, const std::vector<double>& w, int latency,
                           ...
                           uint32_t minSignalThreshold=0, uint32_t PMT_NoiseThreshold=0, bool printCSVFile=false);
...
void HcalTriggerPrimitiveAlgo::run(const HcalTPGCoder* incoder,
   ...
   {
     if (item.first.ieta() == MYIETA && item.first.iphi() == MYIPHI) {
       cout << "in run(), after finishing filling theSumMap, at entry " << MYIETA << " " << MYIPHI << endl;
       cout << "here is the IntegerCaloSample for this entry:" << endl;
